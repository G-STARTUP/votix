    ...
    public function generateValidationRules($kyc_fields) {
        ...
        foreach($kyc_fields ?? [] as $item) {
            ...
            if($item->type == "text" || $item->type == "textarea") {
                ...
                // Optional regex pattern support if provided in validation object
                $pattern = $item->validation->regex ?? null;
                if($pattern) {
                    // Ensure pattern is wrapped with delimiters
                    $delimited = $pattern;
                    if(!preg_match('/^\/.+\/[a-zA-Z]*$/',$pattern)) {
                        $delimited = '/'.$pattern.'/';
                    }
                    $validation_rules[$item->name] .= '|regex:'.$delimited;
                }
